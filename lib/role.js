...
class Role {
    ...
    async viewAll() {
        try {
            const rows = await db.query('SELECT d.id AS department_id, d.name AS department_name, r.id AS role_id, r.title AS role_title, r.salary AS role_salary FROM roles r LEFT JOIN departments d ON r.department_id = d.id ORDER BY d.name, r.title ASC');
            if (rows[0].length !== 0) {
                console.log('***** Roles *****\n');
    ...
    insertNew() {
        ...
        .then(async res => {
            ...
            try {
                const newRole = await db.query('INSERT INTO roles SET ?', role);
                return role;
            } catch (err) {
                console.log(err);
                return err;
    ...
    updateExisting() {
        ...
        .then(async res => {
            try {
                const updateRole = await db.query(`UPDATE roles SET ${res.roleField} = ? WHERE id = ?`, [res.newValue, res.roleId]);
        ...
        .then(async res => {
            if (!isNaN(res.roleId)) {
                try {
                    ...
                    const deleteRole = await db.query('DELETE FROM roles WHERE id = ?', res.roleId);
